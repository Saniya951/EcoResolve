import 'package:eco_resolve/comapis/com_apis.dart';
...
class ComchatController extends GetxController{
  ...
  final list = <ComMessage>[
    ComMessage(commsg: 'Hello! How can I assist you today?', comMsgType:ComMessageType.bot2)
  ].obs;

  Future<void> askQuestion() async{
    if(textC.text.trim().isNotEmpty){
      //user
      list.add(ComMessage(commsg: textC.text, comMsgType:ComMessageType.user2));
      list.add(ComMessage(commsg: 'something went wrong', comMsgType: ComMessageType.bot2));
      _scrollDown();
